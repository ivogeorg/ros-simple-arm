...
std::vector<float> clamp_at_boundaries(float requested_j1, float requested_j2) {
    ...
    // Assign a new node handle since there's no access to the main one
    ros::NodeHandle n2;
    ...
    // Check if requested joint 1 angle falls in the safe zone, else clamp
    ...
    // Check if requested joint 2 angle falls in the safe zone, else clamp
    if (requested_j2 < min_j2 || requested_j2 > max_j2) {
        clamped_j2 = std::min(std::max(requested_j2, min_j2), max_j2);
        ROS_WARN("j2 is out of bounds, valid range (%1.2f, %1.2f), clampint to: %1.2f", min_j2, max_j2, clamped_j2);
    }
...
// Callback executes whenever a safe_move service is requested
bool handle_safe_move_request(simple_arm::GoToPosition::Request& req, simple_arm::GoToPosition::Response& res) {
    ...
    // Check if requested joint angles are in the safe zone, else clamp them
...
int main(int argc, char** argv) {
    ...
    // Define two publishers to publish std_msgs::Float64 messages to joints' respective topics
    ...
    // Define a safe_move service with a handle_safe_move_request callback function (handler)
