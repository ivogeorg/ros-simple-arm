...
// Callback continuously executes and reads the arm joint angles position
...
// This callback function continouosly executes and reads the image data
void look_away_callback(const sensor_msgs::Image img) {
    bool uniform_image = true;
    
    // Loop through each pixel in the image and check if it's equal to the first one
...
int main(int argc, char** argv) {
    ...
    // Subscribe to /simple_arm/image_raw topic to read the image data inside the look_away_callback function
    ros::Subscriber sub2 = n.subscribe("rgb_camera/image_raw", 10, look_away_callback);
    
    // Handle ROS communication data
    ros::spin();
